...
let citydata = [
  ...
  let usefulStats = []; //created an array for all the city stats
  let warmestTemp = 0; //created variable for warmest temp
  let warmestCity = {};//created an object for warmest city
  //created for loop to pull all the specific data needed from city data
  for (let i = 0; i < citydata.length; i++) { 
    let newCity = {   //made a new object for new city, and pulled all data with dot.notation 
    ...
    usefulStats.push(newCity); //pushed all the stats from new city into the array for useful city stats. 
  }
  
  for (let j = 0; j < usefulStats.length; j++) { // created forloop for useful stats - this is for the next four hour temp for all cities
